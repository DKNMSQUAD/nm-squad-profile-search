export default function CompanyCard({ company, criteria, selectedCriteria, onPreview, isActive }) {
  ...
  return (
    <div
      onClick={() => onPreview(company)}
      style={{
        background: '#fff',
        border: isActive ? '2px solid #1a1a1a' : '1px solid #d9d0c0',
        borderRadius: 2,
        padding: isActive ? '17px 19px 15px' : '18px 20px 16px',
        transition: 'box-shadow 0.15s, border 0.15s',
        cursor: 'pointer',
      }}
      onMouseEnter={e => { if (!isActive) e.currentTarget.style.boxShadow = '0 2px 12px rgba(0,0,0,0.1)' }}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {hasWebsite ? (
            ...
            <span style={{ fontFamily: 'IBM Plex Mono', fontSize: 9, color: '#ccc', letterSpacing: '0.04em', display: 'block', marginTop: 2 }}>
              no website
            </span>
          )}
        </div>
